...
connectSocket(){try{this.socket=io();this.socket.on('connect',()=>{console.log('💬 Admin connected to chat server');this.isConnected=true;this.updateConnectionState();});this.socket.on('disconnect',()=>{console.log('💬 Admin disconnected from chat server');this.isConnected=false;this.updateConnectionState();});this.socket.on('chat_history',(data)=>{this.renderChatHistory(data.history);});this.socket.on('receive_message',(data)=>{if(data.session_id==this.activeSessionId){this.addMessage(data.message,data.sender_type,data.timestamp);}else{this.showNotificationDot(data.session_id);}});}catch(error){console.error('Failed to connect to chat server:',error);this.isConnected=false;this.updateConnectionState();}}
